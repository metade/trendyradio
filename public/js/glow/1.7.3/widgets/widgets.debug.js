...
(window.gloader || glow).module({
	...
	builder: function(glow) {
		var dom = glow.dom,
			...
			*/
			overlayHtml = '<div class="glow173-overlay glowNoMask"><div class="overlay-focalPoint" tabindex="-1"></div></div>',
		...
		*/
		function Overlay(content, opts) {
			...
			this.opts = opts = glow.lang.apply({
				...
				focusOnShow: false,
				id: "glow173Overlay" + (++overlayCount),
				closeOnEsc: false
			}, opts);
	...
	builder: function(glow) {
			...
			glow.ready(function () {
				var widgetSheet = function (sheets) {
					...
					_ieCssRule = function(theme, className, image) {
						return ".glow173-ie .glow173-overlay" + cssPngThemes[theme].className + " ." +
		...
		*/
		defaultTemplate = function() {
			var r = [], rLen = 0;
			r[rLen++] = '<div class="glow173-panel">';
		...
		function Panel(content, opts) {
			...
			*/
			...
			if (!customTemplate) {
				...
				if (!themesPreloaded[opts.theme] && docBody.className.indexOf("glow173-basic") == -1) {
					fullContentClone = fullContent.clone().addClass("glow173-panel-preload").appendTo(docBody);
...
(window.gloader || glow).module({
	...
	builder: function(glow) {
		var dom = glow.dom,
			...
			env = glow.env,
			win,
			positionRegex = /glow173\-infoPanel\-point[TRBL]/,
		...
		*/
		function InfoPanel(content, opts) {
			...
			if (!customTemplate) {
				this.content.addClass("glow173-infoPanel");
			}

			this.content.addClass("glow173-infoPanel-point" + (opts.pointerPosition || "t").slice(0,1).toUpperCase());
		}
			...
			setPosition: function(x, y) {
				...
				if (!pointerPosition) {
					...
					if (lastPointerPosition != pointerPosition) {
						...
						contentNode.className = contentNode.className.replace(positionRegex, "glow173-infoPanel-point" + pointerPosition);
...
/* private functions **********************************************************/
		...
		function getParentListItem(that, node) { /*debug*///console.log("getParentListItem("+node+")");
			...
			while (listItem.parentNode && listItem.parentNode.parentNode) {
				if ($(listItem.parentNode.parentNode).hasClass('glow173-autoSuggest')) break;
		...
		*/
		glow.widgets.AutoSuggest = function(inputElement, dataSource, opts) { /*debug*///console.log('new glow.widgets.AutoSuggest('+inputElement+', '+dataSource+', '+opts+')');
			...
			this.overlay = new glow.widgets.Overlay(
				glow.dom.create('<div class="glow173-autoSuggest"><ul></ul></div>'),
				{
					autoPosition: false,
...
/* public methods *************************************************************/
		...
		glow.widgets.AutoSuggest.prototype.configure = function(opts) {
			...
			if (this.opts.height) {
				var listContainer = $(this.overlay.container.get('.glow173-autoSuggest').get('ul')[0]);
		...
		function Carousel(container, opts) {
			...
			this.element =
				dom.create("<div"
					...
					+ " class=\"" + (this._opts.vertical? "glow173-vCarousel" : "glow173-carousel")
			...
			if (this._opts.vertical) {
				this.element.addClass("glow173-vCarousel");
			}
			else {
				this.element.addClass("glow173-carousel");
			}
	...
	builder: function(glow) {
		...
		*/
		...
		glow.widgets.Editor = function(textarea, opts) {
			...
			// interpolate context
			this.element = glow.dom.create('<div class="glow173-editor"><p class="glow173-hidden">{ENTER_MESSAGE}, <a href="#endOfEditor' + endOfEditorCounter() + '" tabindex="0">{SKIP_LINK_TEXT}</a></p><div class="editor-' + (opts.theme || "light") + '"><div class="editor-state"></div></div><p id="endOfEditor' + endOfEditorCounter() + '" class="glow173-hidden endOfEditorCounter" tabindex="0">{LEAVE_MESSAGE}</p></div>', {interpolate : editorLocaleModule});
